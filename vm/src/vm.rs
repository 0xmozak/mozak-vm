...
impl Vm {
    ...
    pub fn step(mut state: State) -> Result<(Self, State)> {
        ...
        while !state.has_halted() {
            let inst = Instruction::from(&state);
            state = state.execute_instruction();
            let processor = processor_row(&inst, &state);
            let memory = MemoryTraceRow::default();
...
pub fn processor_row(inst: &Instruction, state: &State) -> ProcessorTraceRow {
    ProcessorTraceRow {
        ...
        register_selectors: RegisterSelector::from(inst),
        pc: state.pc,
        opcode: inst.into(),
        ..Default::default()
    }
...
mod tests {
    ...
    fn simple_test(exit: u32, mem: &[(u32, u32)], regs: &[(usize, u32)]) -> State {
