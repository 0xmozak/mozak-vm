...
{
    fn add_virtual_u32_target(&mut self) -> U32Target { U32Target(self.add_virtual_target()) }
    ...
    fn zero_u32(&mut self) -> U32Target { U32Target(self.zero()) }

    fn one_u32(&mut self) -> U32Target { U32Target(self.one()) }

    fn connect_u32(&mut self, x: U32Target, y: U32Target) { self.connect(x.0, y.0) }

    fn assert_zero_u32(&mut self, x: U32Target) { self.assert_zero(x.0) }
...
{
    fn id(&self) -> String { format!("split_to_u32_: {:?}", self.x) }

    fn dependencies(&self) -> Vec<Target> { vec![self.x] }
