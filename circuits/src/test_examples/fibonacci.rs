...
// fn test_fibonacci() {
//     let program =
// Program::vanilla_load_elf(mozak_examples::FIBONACCI_ELF).unwrap();
...
//     let program = Program::mozak_load_program(mozak_examples::FIBONACCI_ELF,
// &args).unwrap();     let state =
// State::<GoldilocksField>::new(program.clone());     let record =
// step(&program, state).unwrap();     MozakStark::prove_and_verify(&program,
// &record).unwrap(); }
...
// #[should_panic]
// // NOTE: this test should panic since ELF is expecting to have non-empty
// input fn test_fibonacci_mozak_elf_new_api_empty_args() {
//     let args = RuntimeArguments::default();
//     let program =
