...
pub struct StoreWordStark<F, const D: usize> {
    pub _f: PhantomData<F>,
    pub standalone_proving: bool,
}

impl<F, const D: usize> HasNamedColumns for StoreWordStark<F, D> {
    type Columns = StoreWord<F>;
}

const COLUMNS: usize = StoreWord::<()>::NUMBER_OF_COLUMNS;
const PUBLIC_INPUTS: usize = 0;

fn generate_constraints<'a, T: Copy>(
    vars: &StarkFrameTyped<StoreWord<Expr<'a, T>>, NoColumns<Expr<'a, T>>>,
...
impl<F: RichField + Extendable<D>, const D: usize> Stark<F, D> for StoreWordStark<F, D> {
