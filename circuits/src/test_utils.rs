...
pub fn cuda_ctx() -> CudaInvContext<F, D> {
    ...
    // TODO(Kapil): is this required?
    let _ctx = Context::create_and_push(ContextFlags::MAP_HOST | ContextFlags::SCHED_AUTO, device)
        .unwrap();
    let stream = Stream::new(StreamFlags::NON_BLOCKING, None).unwrap();
    // TODO(Kapil): figure out if this is necessary for now?
    let stream2 = Stream::new(StreamFlags::NON_BLOCKING, None).unwrap();
    // max size of number of rows
    ...
    let shift_powers = F::coset_shift()
        ...
        .take(1 << (lg_n))
        .collect::<Vec<F>>();
    // used for coset fft over lde domain
