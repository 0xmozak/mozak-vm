    ...
    step_rows
        ...
        .map(|row| {
            ...
            Memory {
                ...
                is_lb: F::from_bool(matches!(op, Op::LB)),
                is_lbu: F::from_bool(matches!(op, Op::LBU)),
                is_init: F::ZERO,
                value: F::from_canonical_u32(row.aux.dst_val),
                value_sign_bit: F::from_bool(row.aux.dst_val > 127),
                ..Default::default()
            }
        })
    ...
    [(F::ZERO, &program.ro_memory), (F::ONE, &program.rw_memory)]
        ...
        .flat_map(|(is_writable, mem)| {
            mem.iter().map(move |(&addr, &value)| Memory {
                ...
                value_sign_bit: F::from_bool(value > 127),
                ..Default::default()
            })
        })
        .sorted_by_key(|memory| memory.addr.to_canonical_u64())
...
mod tests {
    ...
    fn generate_memory_trace() {
        ...
        assert_eq!(
            ...
            #[rustfmt::skip]
            prep_table(vec![
                //is_writable  addr   clk  is_sb,  is_lb, is_lbu, is_init  value   value_sign_bit,  diff_addr  diff_addr_inv  diff_clk
                [       1,     100,   0,     0,     0,     0,       1,        0,        0,              100,     inv(100),            0],  // Memory Init: 100
                [       1,     100,   1,     1,     0,     0,       0,      255,        1,                0,           0,             1],  // Operations:  100
                [       1,     100,   2,     0,     0,     1,       0,      255,        1,                0,           0,             1],  // Operations:  100
                [       1,     100,   5,     1,     0,     0,       0,       10,        0,                0,           0,             3],  // Operations:  100
                [       1,     100,   6,     0,     0,     1,       0,       10,        0,                0,           0,             1],  // Operations:  100
                [       1,     101,   0,     0,     0,     0,       1,        0,        0,                1,      inv(1),             0],  // Memory Init: 101
                [       1,     102,   0,     0,     0,     0,       1,        0,        0,                1,      inv(1),             0],  // Memory Init: 102
                [       1,     103,   0,     0,     0,     0,       1,        0,        0,                1,      inv(1),             0],  // Memory Init: 103
                [       1,     200,   0,     0,     0,     0,       1,        0,        0,               97,     inv(97),             0],  // Memory Init: 200
                [       1,     200,   3,     1,     0,     0,       0,       15,        0,                0,           0,             3],  // Operations:  200
                [       1,     200,   4,     0,     0,     1,       0,       15,        0,                0,           0,             1],  // Operations:  200
                [       1,     201,   0,     0,     0,     0,       1,        0,        0,                1,      inv(1),             0],  // Memory Init: 201
                [       1,     202,   0,     0,     0,     0,       1,        0,        0,                1,      inv(1),             0],  // Memory Init: 202
                [       1,     203,   0,     0,     0,     0,       1,        0,        0,                1,      inv(1),             0],  // Memory Init: 203
    ...
    fn generate_memory_trace_only_init() {
        ...
        assert_eq!(trace, prep_table(vec![
            // is_writable   addr   clk  is_sb, is_lb, is_lbu, is_init   value  value_sign_bit   diff_addr  diff_addr_inv  diff_clk
            [        0,      100,   0,      0,    0,    0,        1,       5,     0,                100,    inv(100),             0],
            [        0,      101,   0,      0,    0,    0,        1,       6,     0,                  1,           1,             0],
            [        1,      200,   0,      0,    0,    0,        1,       7,     0,                 99,     inv(99),             0],
            [        1,      201,   0,      0,    0,    0,        1,       8,     0,                  1,           1,             0],
        ]));
    }
}
