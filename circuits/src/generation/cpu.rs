...
fn generate_mul_row<F: RichField>(row: &mut CpuState<F>, inst: &Instruction, aux: &Aux) {
    ...
    row.product = F::from_noncanonical_i64(res);
    let low_bits = (res & 0xFFFF_FFFF) as u32;
    let high_bits = (res >> 32) as u32;
    row.product_low_bits = from_u32(low_bits);
    row.product_high_bits = from_u32(high_bits);

    // Prove that the high limb is different from `u32::MAX`:
    let high_diff: F = from_u32(u32::MAX - high_bits);
