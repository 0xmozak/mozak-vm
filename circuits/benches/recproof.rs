...
use mozak_circuits::recproof::{hash_branch, BranchCircuit, LeafCircuit, Object};
...
fn bench_prove_verify_recproof(c: &mut Criterion) {
    ...
    let branch_ab = black_box(hash_branch(object_a_v1_pair.1, object_b_pair.1));
    let branch_ab_v2 = black_box(hash_branch(object_a_v2_pair.1, object_b_pair.1));
    ...
    group.bench_function("recproof_branch_verify", |b| {
        b.iter(|| branch_circuit.circuit.verify(branch_proof.clone()).unwrap())
    });
    group.finish();
}
